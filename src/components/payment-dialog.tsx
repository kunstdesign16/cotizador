...
export default function PaymentDialog({ open, setOpen, orderId, orderTotal }: PaymentDialogProps) {
    ...
    const handleSubmit = async () => {
        if (!orderId || numericAmount <= 0) {
            alert('Por favor ingrese un monto válido');
            return;
        }

        // We pass the BASE amount and the IVA separately
        const result = await registerPayment({
            orderId,
            type,
            amount: base, // Database expects the subtotal/base amount
            iva: iva
        });
